    ...
    def compute_loss(self, model, inputs, return_outputs=False, num_items_in_batch=None):
        ...
        if use_image:
            ...
            for x in inputs:
                ...
                video_path = x["video"]
                video_duration = get_video_duration(video_path)  # 获取视频时长
                
                if video_duration is not None:
                    fps =int(8 / video_duration) # 动态计算fps
                else:
                    fps = 2  # 默认值，如果无法获取视频时长则使用最小fps值
        ...
        if use_video:
            per_token_logps = self._get_per_token_logps_video(model, prompt_completion_ids, attention_mask, pixel_values_videos, video_grid_thw)
        ...
        with torch.inference_mode():
            if self.ref_model is not None:
                ...
                if use_video:
                    ref_per_token_logps = self._get_per_token_logps_video(self.ref_model, prompt_completion_ids, attention_mask, pixel_values_videos, video_grid_thw)
            else:
